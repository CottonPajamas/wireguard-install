...
function installQuestions() {
	...
	SERVER_PUB_IP=$(curl http://169.254.169.254/latest/meta-data/public-ipv4)
	if [[ -z ${SERVER_PUB_IP} ]]; then
		# Detect public IPv6 address
		SERVER_PUB_IP=$(curl http://169.254.169.254/latest/meta-data/public-ipv6)
	fi
	echo "IPv4 or IPv6 public address: " $SERVER_PUB_IP
	...
	echo "Server's WireGuard IPv4: " $SERVER_WG_IPV4
	
	SERVER_WG_IPV6=fd42:42:42::1
	echo "Server's WireGuard IPv6: "$SERVER_WG_IPV6

	# Generate random number within private ports range
	SERVER_PORT=$(shuf -i49152-65535 -n1)
	echo "Server's WireGuard port [1-65535]: " $SERVER_PORT
	...
	echo "Second DNS resolver to use for the clients (optional): " $CLIENT_DNS_2
